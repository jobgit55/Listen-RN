...
interface AlbumComponentProps extends ModelState {
  headerHeight: number;
...
class Album extends React.Component<AlbumComponentProps> {
  ...
  translateY = Animated.add(
    Animated.add(this.translationY, this.translationYOffset),
  ...
  onItemPress = (data: ProgramProps, index: number) => {
    const {navigation, dispatch, list, route} = this.props;
    const perviousItem = list[index - 1];
    const nextItem = list[index + 1];
    dispatch({
      type: 'player/setState',
      payload: {
        previousId: perviousItem ? perviousItem.id : '',
...
function Wrapper(props: AlbumComponentProps) {
